...
const CountriesByFlag = ({ countryFilter, isDarkTheme, isError, error, isLoading, countries }) => {
                ...
                {isLoading
                    ...
                    : isError
                        ...
                        : countries.filter(x => x.name.toLowerCase().startsWith(countryFilter)).map((item, key) =>
                            <FlagBox key={key} item={item} handleShow={handleShow} />
                        )
                }
            </Row>
...
export default CountriesByFlag